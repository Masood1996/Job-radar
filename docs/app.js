const COMPANY_URL='../data/companies.json';let companies=[],jobs=[];const KEY='pharmaRadarState';const COMPANY_KEY='pharmaRadarImportedCompanies';
...
async function load(){const base=await fetch(COMPANY_URL).then(r=>r.json());const imported=getImportedCompanies();const seen=new Set(base.map(companyId));const extra=imported.filter(c=>!seen.has(companyId(c)));companies=base.concat(extra);const saved=JSON.parse(localStorage.getItem(KEY)||'{}');jobs=(saved.jobs||[]);document.querySelector('#companies').textContent=companies.length;document.querySelector('#scan').textContent=saved.lastScan?new Date(saved.lastScan).toLocaleString():'Not yet';render();}
